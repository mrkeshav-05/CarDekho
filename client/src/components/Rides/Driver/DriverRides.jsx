...
import styled from "styled-components";
import Navbar from "../../../components/Navbar/Navbar.jsx";
import TripCard from "../../../components/Card/trip.jsx";
...
function DriverRides({ user, setIsLoggedIn, setCurrentChat, currentChat }) {
  const [trips, setTrips] = useState([]);
  ...
  const mockTrips = [
    {
      _id: "1",
      time: new Date(new Date().getTime() + 60 * 60 * 1000), // 1 hour from now
      name: "Trip to the airport",
    },
    {
      _id: "2",
      time: new Date(new Date().getTime() - 60 * 60 * 1000), // 1 hour ago
